...
def game ( p,  c,  set, enemy ):
    ...
    p_choice = input ("Choose Rock, Paper, or Scissors. Type 'scout' for more information on " + opp.name + " ")
    while (p_choice.lower() != "rock") and (p_choice.lower() != "paper") and (p_choice.lower() != "scissors"):

        if (p_choice.lower() == "scout"):
             print ( opp.tendency() )
            
        p_choice = input ("Choose Rock, Paper, or Scissors.  Type 'scout' for more information on " + opp.name + "  ")
    
    print ("You did " + p_choice)
    print (opp.name + " did " + c_choice)
    print()
    if (p_choice.lower() == "rock"):
        p.r_played += 1
    if (p_choice.lower() == "paper"):
        p.p_played += 1
    if (p_choice.lower() == "scissors"):
        p.s_played += 1
